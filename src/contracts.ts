...
const toolContractsBase = {
  ...
  abap_activate: {
    description:
      "Activate one explicit Z* or Y* ABAP object URI without resaving source. Success requires active-source readback to match the reviewed candidate. Include activation: this release (SAP_BASIS 7.31) publishes no way to ask an include for the main program that pulls it in, and SAP refuses an include whose registration it cannot match ('Main program is not anymore valid for include ...'). An include that no program references yet activates normally. Activate the including program instead when the include is already referenced: the program's own source is read for its INCLUDE list and those drafts are activated with the program as their context, which is reported as INCLUDE_GRAPH_ACTIVATED - or as INCLUDE_GRAPH_UNVERIFIED when that list could not be read. Never treat a program's successful activation as proof that its includes are active when the receipt says the graph was not verified. Returns activation errors and leaves transport release to the user.",
    inputSchema: {
      ...writeOperationInput,
      url: z.string()
    }
