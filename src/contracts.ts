...
const toolContractsBase = {
  ...
  execute_data_query: {
    description:
      "Run a read-only ABAP SQL SELECT through SAP ADT and return bounded JSON text. Standalone mode supports displayMode=internal only; rowRange is mandatory and at most 1000 rows. UI, files, webviews, direct data, and mutations are rejected. Every table in the statement must be in the D5-2 allowlist (default deny) on both the native and the fallback path; a statement whose tables cannot be enumerated (dynamic table name, no FROM, comma-joined table list) is rejected before any SAP access with TABLE_ALLOWLIST_UNVERIFIABLE. When the platform does not serve the native data preview endpoint (its known empty-HTML error), the statement runs over the finite fallback dialect instead: one table; SELECT * or a column list; a WHERE of up to 8 comparison groups joined by OR, each group up to 8 comparisons joined by AND over =, <>, <, <=, >, >=; and an optional ORDER BY of up to 8 keys. Each OR group is one server-side read, so querySource reports disjuncts, deduplicatedRows (rows collapsed only when a whole-row read identifies them), repeatedProjectedRows (with a partial projection identical values prove nothing, so such rows are kept and counted) and incompleteBranches (groups that stopped at the row bound, leaving a page rather than the whole match set). ORDER BY is refused with TABLE_QUERY_ORDER_BY_INCOMPLETE unless every group was read completely, and with TABLE_QUERY_ORDER_BY_COLUMN_NOT_SELECTED unless its keys are projected; it orders by the reader's text values, which is not SAP's type-aware ordering. Joins, aggregates, GROUP BY, expressions, aliases, subqueries and LIMIT are never translated.",
    inputSchema: {
