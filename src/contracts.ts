...
const toolContractsBase = {
  ...
  replace_string_in_abap_object: {
    description:
      "Edit a Z* or Y* customer source by exact unique string replacement. Supported targets: classes, interfaces, programs, includes, function groups, function modules, function-group includes, DDL sources, and DCL sources. Standard owners and children are rejected before locking. A function module is written through the SAP side function write instead of the ADT source path, because SAP_BASIS 7.31 refuses the function module source PUT with HTTP 423 whatever resource is locked; that target therefore also requires repository helper protocol 2.7 (WRITE_FUNCTION_SOURCE), it takes no ADT lock, and it can only replace the implementation body - a replacement that reaches the FUNCTION ... interface section or ENDFUNCTION is refused and must be applied with patch_function_module_interface or in SE37. The service fails closed when inactive state is unavailable and rejects pre-existing inactive source by default. Set recoverInactiveSource=true only to repair a reviewed inactive draft; expectedSourceFingerprint is then required and must match that draft under the native SAP lock. The service saves with an explicit or existing transport, unlocks, and activates. Include activation uses an unambiguous SAP-provided main-program context. A saved-but-unverified result includes stage and source-fingerprint evidence and must not be retried as another replacement. It never creates or releases transports.",
    inputSchema: {
      ...writeOperationInput,
      fileUri: z.string(),
      oldString: z.string(),
