...
- On the known empty-HTML error, execute_data_query can reuse that reader for SELECT * (or comma-separated plain fields) FROM one table WHERE <field> <op> <value> [AND ...], with explicit maxRows <=500. Translated comparisons are =, <>, <, <=, >, >=; at most 8 conjuncts joined by AND; a value is a single-quoted literal (double a quote to escape it) or a bare number. No joins, aliases, expressions, subqueries, OR, ORDER BY, aggregates or functions are translated. Original ADT errors for all other queries remain errors.
