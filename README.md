...
2026-09-23 版本 `0.47.2` 修复 **15:20 恢复激活事件**：`resume_ddic_table_activation` 对一个已保存但未激活的完整新表返回 `WORKLIST_REQUIRED: Expected conversion worklist is required`，而同一时刻 `read_ddic_table_conversion_status` 报 `pending=false`、`entries=[]`，能力报告还把该能力报为 `available`。**这是"能派发但从不执行"的一整条链**，四项根因：①DDIC 助手的 `WHEN 'RESUME_TABLE_ACTIVATION'` 同时置 `lv_recover='X'`，使恢复激活进入 TBATG 转换恢复块——无工作清单时回 `WORKLIST_REQUIRED`，**有工作清单时执行一次转换恢复**（`DD_DB_CONVERTER`，并在调用方传输上重新记录对象），本操作自己的 `lv_resume` → `DD_TABL_ACT` 分支因此从未执行；②即使修正派发，该分支仍被共享写路径**四道更早的闸门**拦住，其中 `lv_gotstate <> 'A'` 分支（即"只有非活动版本"这一**正是恢复场景**的处境）会在无版本令牌时执行 `DDIF_OBJECT_DELETE` **删除待激活的定义**，其余三道为 `DDIC_OBJECT_EXISTS` / `EXPECTED_VERSION_REQUIRED` / `VERSION_CONFLICT`；③注册表把 `minHelperProtocol` 钉在 `1.11`，而 1.11/1.12 助手虽能派发该操作码却从不执行它——**版本存在被当成能力可用**；④服务端把 64 位定义内容指纹当作助手的 `expectedVersion` 传入，而该形参的语义是**活动版本时间戳令牌**（`AS4DATE`+`AS4TIME`，14 位），两者永不相等（该错配一直被根因①掩盖）。**修复**：派发只置 `lv_write`+`lv_resume`；`lv_gotstate <> 'A'` 分支新增保留臂（恢复绝不进入删除重置）；三道写闸门与写输入守卫各加 `lv_resume` 豁免；DDIC 能力表行与分支 `ev_version` 抬到 `1.13`，注册表 `minHelperProtocol` 同步 `1.11 → 1.13`；恢复调用不再传 `expectedVersion`，其陈旧读保护留在服务端 `expectedInactiveFingerprint` 门。**DDIC 助手协议 `1.12 → 1.13`（33 个操作码不变），须在 SE38 执行 F8 部署；部署前该能力会正确显示 `unsupported`（version-check），这是预期的诚实状态而非回归。** 新增 3 项回归（派发结构、闸门豁免、调用参数），每条均演示过"改回缺陷即失败"；生成正文 5,145 行经离线块结构检查为 BALANCED（检查器以"删掉一个 ENDIF"副本证伪）。**工具面不变**（137 项 / 只读 79 项 / 能力组 12 组 / 能力项 21）。**本地门禁**：`npm run verify` exit 0、**806 项测试全部通过**。**本版本未修改任何 SAP 标准对象，未释放传输 `GR2K923472`，未执行 SAP 运行时验证。**

2026-09-23 版本 `0.47.1` 是**进度版本，不是覆盖率里程碑**：它收口 D6-5（追加结构的读与写）并补齐治理面，**不携带任何「SAP ECC ABAP 日常开发 95% 场景已覆盖」的声明**——该判定须先有 P0-3（调试器端点）与 P0-4（ADT RIS 配置）的 Basis 结论。**工具面 136 → 137 项（只读 79 项；能力组 12 组，能力项 20 → 21）**（实测 `tool index consistent: 137 tools, 79 read-only, 12 groups`、`helper capabilities consistent: 21 capabilities over 55 tools`）。**D6-5 追加结构**：只读 `read_append_structure` 现发布**基表**（此前只给字段列表），写入 `upsert_append_structure_fields` 追加/删除字段（DDIC 助手 `1.12`，**33 个操作码**，`bootstrap DDIC capability table declares every required operation: 33 dispatched over 33 declared`）。随本项修复 5 个缺陷：N-2（**非活动**版本的结构不返回字段）、`AS4LOCAL`/`AS4VERS` 被误写入 `DD03P` 追加行、追加字段列表前导分隔符、search-help 写入前先清空既有行、`$TMP` 包写入被传输要求挡下。另修「契约未声明的入参被静默丢弃」：未声明参数现在**拒绝**而不是忽略。**治理**：`get_capability_report` 新增**证据维度**（顶层 `verification` 块 + 每个能力与每个版本化助手族的 `verification` 汇总 + `availabilityWithoutEvidence`），数据源为 `contracts/verification-registry.json`；**验收状态永不反向影响可用性判定**——`available + unverified` 是正常状态而非缺陷，登记表读不到时一律降级为 `unverified` 而不是声称已验证（S4/R-8 落地）。`release/` 保留窗口由「只警告」改为调用 `scripts/quarantine-release-artifacts.ps1` **移动**超窗产物到 `release/quarantine/`（可逆、不删除），本次执行移入 46 条路径。R-14 的两个偶发失败测试**已定位根因**：测试用 `listen(0)` 取端口，而本机动态端口范围 `1024–15000` 与 Node/undici 保留端口表重叠，`fetch` 会拒绝该端口；现 13 处绑定统一经 `test/loopback-port.ts` 取端口。R-10 客户常量集中到 `src/customer-scope.ts`（连接 `w200`／客户端 `200`、`ZTPMC_BZWL` 的 `809P` 作用域读取、白名单引用），并明确「换客户是部署变更，不是配置开关」。`docs/release-process.md` 补 §7：SAP 侧助手的**人工 F8 部署步骤**、两条协议刻度（DDIC `1.x` / repository `2.x`）、载体顺序不变量与指纹重钉规则。**本地门禁**：`npm run verify` exit 0、**802 项测试全部通过**（`tool index consistent: 137 tools, 79 read-only, 12 groups`）。**仍未完成**：D6-1 缺陷②（`HIDEFLAG`/长描述下 `DD31S`/`DD33S` 行被写入侧丢弃 = 横切 R-4）、D7、D8、D9；R-15/F-4（repository 助手回执字段与 `FUNC` 分配查询）因需 repository 助手载体而留在下一批；R-12 的 CI 工作流**仍未在真实 runner 上运行**。**本版本未修改任何 SAP 标准对象，未释放传输 `GR2K923472`。**

2026-09-23 版本 `0.46.15` 发布 **D6-3 编号范围对象**与 **D6-4 维护视图**，并收口 R-19 / R-20 / R-20c / R-12 / S4。**工具面 132 → 136 项（只读 77 → 79 项），能力组 18 → 20 组**；`docs/tool-index.md` 与 `contracts/tool-index.json` 由注册表重新生成。**助手侧**：DDIC 助手 `PROTOCOL|MAX` 由能力表**推导**为 `1.11`、共 **32 个操作码**，已在 `w200` 部署并自述（正文 sha256 `8d47fd23…`，载体 r24；`1.11` 组 7 行 = 1 个改名 resume + 3 个号码范围 + 3 个维护视图）。**D6-3**：服务侧 `read_number_range_object` / `upsert_number_range_object` + `delete_ddic_object(objectType=NROB)`；范围仅 `TNRO` + `TNROT`，**区间值（`NRIV`）明确不在范围内**；并发令牌是 40 字符 SHA-1 **定义摘要**（`TNRO` 无 `AS4DATE`/`AS4TIME`），不是 DDIC 时间戳；运行时闭环已完成（`ZORVMCPNR1` 创建 → 删除 → 读回不存在）。**D6-4**：`read_maintenance_view` / `upsert_maintenance_view`；视图语义的权威来源是 `DD_VIEW_GET` 返回的 `DD25V`/`DD26V`/`DD27P`/`DD28V`，故 N-1（`DD02V` 读不到字段）随本项交付关闭；运行时闭环已完成（`ZORV_MCP_V01` 创建 v`20260922172217` → 删除 → `read_maintenance_view` 返回 `DDIC_OBJECT_NOT_FOUND`）。**已知平台限制**：客户锁对象（`ENQU`）的 `TADIR` 注册被 SAP 标准包检查拒绝（`TADIR_ENTRY_FAILED`），`upsert_lock_object` 无法创建可传输的注册，已失败关闭并文档化于 `docs/ddic-lock-object-limitation.md`——锁对象需人工在 SE11 创建并指定请求。**横切**：R-19（真实内网端点与账号从受跟踪文件、打包占位符与全部验证脚本中脱敏）、R-20/R-20c（操作码改为 ≤32 字符、注册表补 `requiredOperations`、不可投递操作在调用前拒绝且回执不再声称 `outcomeMayBeUnknown`）、R-12（新增 `.github/workflows/verify.yml`，**尚未在真实 runner 上运行**）、S4（`contracts/verification-registry.json` 验收登记表）。**本地门禁**：`npm run verify` exit 0、**798 项测试全部通过**（`tool index consistent: 136 tools, 79 read-only, 12 groups`）。**仍未完成**：D6-1 的**缺陷②**（`HIDEFLAG`/长描述下 `DD31S`/`DD33S` 行被写入侧丢弃，工具报失败但对象已写入 = 横切 R-4）、D6-5、D7、D8、D9。**缺陷①「描述截断」已于 2026-09-20 16:14 修复**（人工 SE37：`IV_DESCRIPTION` 的引用类型 `TSTCT-TTEXT` → `DD04T-DDTEXT`，即 `AS4TEXT` CHAR 60；`interfaceFingerprint` 由 `06b08763…` 变为 `70662f30…`），见 `.doc/code-update-20260920-161418.md` 与 `…-163331.md`。**本版本未修改任何 SAP 标准对象，未释放传输 `GR2K923472`。**
...
**R-17（Medium，可诊断性）**：维护诊断族三只工具（`search_sap_locks`／`search_update_records`／`get_update_record_detail`）的"未批准"回执过去只有裸 `code=HELPER_NOT_APPROVED`，既不说明缺的是**本机批准文件**、也不给期望路径，导致 15:34 与 16:25 两次事件都被误判为"SAP 侧未批准"。**修订**：新增 `reason`（`APPROVAL_FILE_MISSING`／`CONNECTION_NOT_APPROVED`／`SOURCE_NOT_ENABLED`）、`expectedApprovalFile`（本服务**实际读取**的绝对路径），以及仅在来源未启用时出现的 `requestedSource`／`approvedSources`；`execute()` 由"裸 code"改为判别式 `{ failure }` 返回，未来新增失败原因无法绕过 `reason`；指纹、重复条目、批准文档类失败仍抛稳定错误，不降级为 `unavailable`。同族的能力报告 note 与 `docs/helper-capabilities-protocol.md` 中"助手未部署"的误述一并更正为"部署 / 指纹批准 / 本地批准文件是三道独立闸门"，并明确指纹只证明定义未变、不等于无并发锁。**该字段为新增，不改变既有 `code` 取值，调用方无需改动。**

**门禁解封流程与脚本（16:25 事件闭环）**：事件根因是**本机批准文件缺失**（状态根 `%LOCALAPPDATA%\ABAP MCP Standalone\state` 下无 `maintenance-diagnostic-approvals.json`），服务在访问 SAP 前即拒绝，与助手部署无关——`Z_ORVANTA_MAINT_READ` 已部署、远程启用、非 update-task。**新增 `scripts/prepare-maintenance-approval.mjs`**：连接身份取自服务实际使用的 `connections` 文件（`ABAP_MCP_CONFIG` → 仓库 → `release/` 最新解包目录 → 用户级目录，跳过 `sap.example.invalid` 工厂占位），双指纹用 `read_function_module_interface` 读取现网助手得到（与门禁内部同一 reader、同一算法；助手组/远程/update-task/指纹格式任一不符即拒绝），默认 **dry-run**，`--write` 落盘并保留其它连接条目，`--verify` 用同一入参复测。**实测**：写入后同一入参（`DD02L`、`maxResults 100`、用户 `WYS`）在 **0.46.8 包服务（4847）与开发服务（4848）均返回 `status=ok`、`code=OK`、`entries=[]`、`returnedCount=0`**。口径同时更正：成功是 `status=ok` + `entries` **数组**（空数组＝无匹配锁，不是状态未知），`available` 属失败词汇；文件**每次调用重读**，无需重启，删除即撤销。文档见 `docs/maintenance-diagnostics.md` 新增章节。

**DDIC 助手正文回移（High，防止能力回退）**：生成载体 #2 时暴露出一个错误假设——脚本正文并非总领先线上。把线上 1.10 include 与规范正文做 token 级比对，**46 条线上独有行中有 6 处真实差异**（2 处为 `SOURCE|HASH` 摘要，4 处为行为差异）：①非活动定义的表头属性在线上取自 `ls_current_dd02v-*`（`state='M'` 刚读到的表头），脚本却读 `ls_dd02v-*`（早先 `state='A'` 读入的**活动**表头）→ 非活动描述会配上活动版本的描述/类别，与 `INACTIVE=X` 自相矛盾；②锁对象载荷缺少 `ENQMODE`（E/S/X 锁模式）。两处修复当时直接在 SAP 侧载体内完成、未回移。**修订**：在 `New-DdicFunctionSource` 真正归属点回移两处（活动路径 `ls_dd02v-*`/`'H'` 行**刻意不动**），并固化两道守卫：生成器新增 **token 级回归守卫**——任何线上独有代码行的特征 token 在规范正文中缺失即**拒绝生成**（`--accept-live-differences` 仅供人工复核后放行；回移前它以 4 行命中失败，回移后报"无缺失行"）；`test:bootstrap` 新增 4 个回移标记 + 2 个对照标记断言。

**载体 #2（未部署，产物已生成并自检）**：`scripts/export-ddic-helper-source.ps1`（只读 AST 抽取规范正文，校验 72 列/20 宽空白块/26 操作码/两处写路径实现/`PROTOCOL|MAX|1.10`）+ `scripts/generate-ddic-lock-carrier.mjs`（复刻安装器的 `SOURCE|HASH` 计算；经 MCP 只读预检线上首末行、协议版本、写操作缺失、读操作在位、自身陈述行逐项比对；输出 in-SAP 报告，内建幂等、已含 26 操作码即拒、**基线行数锁定 2119**、失败回滚、事后 ≥26 能力行校验）。实测线上 **2119 行**、基线摘要 `8364d74866531cfb`（等于已部署 1.10 载体记录的 `c_digest`），HELPER/PROTOCOL/PACKAGE/TRANSPORT 五项自身陈述行与线上**全部一致**；回移后重新生成的载荷 **2261 行**、摘要 `e80a1fba7a061d09`、`SOURCE|HASH` `12b26f01f229ef0c…`。**本版本未部署任何载体、未执行 F8、未创建批准文件（批准由操作者按上述脚本显式发起）**。线上助手仍自述 24 个操作码，补齐 26 需人工 F8 之后复核 `get_capability_report`。

**本地闸门**：`npm run verify` **exit 0**：prettier、headless lint、`tsc --noEmit`、`matrix:check`（132 工具/77 只读/12 组；18 能力/50 工具；`26 dispatched over 26 declared`）、**751/751 测试**、`test:bootstrap`、`test:installer`、`test:setup`、`verify:headless`。

2026-09-21 版本 `0.46.8` 修好两项**开发工具链缺陷**并把它们变成闸门（R-16、R-2 本地半程），**服务对外行为与工具面完全不变**（132 项 / 只读 77 项 / 能力组 18 组；助手路由与操作码需求未改）。**R-16（High，工具链）**：`npm run verify` 自 `0.46.2` 起**不可能通过**——`test:bootstrap` 一直红，且 `scripts/bootstrap-sap-helper.ps1` 这条"重建助手"路径会**先抛自身守卫**（`Generated function source exceeds 72 characters`）而产不出 payload，即脚本已无法重建助手源码。用 `git worktree` 逐版本二分定位：`7fbe6a0`（0.46.1）PASS → `afa7445`（0.46.2）FAIL（72 字符守卫）→ `ccfc596`（0.46.4）与 HEAD FAIL（字段元数据断言），故回归属 0.46.2 的脚本改动，与 0.46.7 批次无关。两个独立缺陷：①生成源有 4 行超 72 字符（81/81/77/74，最长 `ev_message = 'Derived or server-controlled search help property'.`）→ 改用 `CONCATENATE` 与续行 `IF` 拆分，**消息文本逐字不变**（错误契约不变），且续行缩进压到 20 列以内，因为生成器按 20 字符切块并**拒绝"整块为空白"的块**（首次修复用 28 列缩进即被该守卫拒绝）；②字段元数据断言硬编码每属性总数 `== 2`，而 0.46.3/0.46.4 新增非活动版本读取路径后 `DATATYPE`/`LENG` 合法地变为 3（结构读取器 1 + 表读取器 2）、`COMPTYPE` 为 2 → 断言改为**按发布者**表述（`<ls_field>` 1 处、`ls_dd03p` 2 处）。修复后 `npm run verify` **exit 0**：prettier、headless lint、`tsc`、`matrix:check`、**750/750 测试**、`test:bootstrap`、`test:installer`、`test:setup`、`verify:headless`——这是 0.46.2 以来该链路首次全绿。**R-2（本地半程）**：只读核实脚本中的锁对象写路径**已完整存在**（`UPSERT_LOCK_OBJECT` → `DDIF_ENQU_PUT`（`dd25v_wa`+`dd26e_tab`+`dd27p_tab`）→ `DDIF_ENQU_ACTIVATE`；`DELETE_LOCK_OBJECT` → `RS_CORR_INSERT mode='D'` + `DDIF_OBJECT_DELETE authority_check='X'`，有引用即 `DEPENDENCIES_EXIST` 回滚），并新增静态闸门：`matrix:check` 读取脚本中的标记块 `ORVANTA-DDIC-CAPABILITY-TABLE`，把注册表为 `Z_ORVANTA_MCP_DDIC_API` 工具钉住的 `requiredHelperOperations` 求并集，任一被派发的操作码未在脚本声明即失败（实测 `26 dispatched over 26 declared`；**失败路径亦实测**：删掉 `UPSERT_LOCK_OBJECT` 后报 `does not declare operation codes the registry dispatches: UPSERT_LOCK_OBJECT`）。**仍未完成的一半（需人工）**：线上助手自述 **24** 个操作码而脚本声明 26，二者相等只能靠**载体 #2 部署 + 人工 F8**，且载体必须在前一个载体 F8 成功后按线上源码重新生成——**本版本未生成、未部署任何载体，未创建批准文件，未调用 SAP**。**同时闭环 15:34 事件**：`search_sap_locks` 返回 `{"status":"unavailable","code":"HELPER_NOT_APPROVED"}` 经只读复核**不是** SAP 侧助手未批准，而是**服务侧本地批准门禁的文档化默认值**（`src/maintenance-diagnostics.ts:193-207` 在读批准文件、访问助手之前即返回；状态根 `%LOCALAPPDATA%\ABAP MCP Standalone\state` 内并没有 `maintenance-diagnostic-approvals.json`），已部署的 `Z_ORVANTA_MAINT_READ` 只是缺 `CAPABILITIES` 版本（`operation-scoped`）；解封路径与"指纹不等于无并发锁"的语义差异见 `.doc/orvanta-mcp-development-followup-plan-20260921.md` §10.3 与 `.logs/mcp-incident-20260921-153457-001-...-closure.md`。工具面不变：132 项 / 只读 77 项，能力组 18 组 / 50 工具。本版本已完成 `tsc` 构建、**750 项自动化测试（全部通过）**与 `npm run verify` 全链路；**未修改任何 SAP 对象、未部署助手、未操作传输**，状态为 `Partially Verified`。

2026-09-21 版本 `0.46.7` 修复两项**已取证的高危缺陷**（能力误报 R-1、白名单绕过 S1/Q10），并补齐发布溯源（S3）。**缺陷 R-1（能力误报，High）**：`get_capability_report` 只用助手自述的**协议版本**判定能力可用性，而协议版本只说明"实现了哪个版本"，不说明"实现过哪些操作码"。实测已部署的 DDIC 助手自述 `maxProtocol 1.10`、24 个操作码，而 `UPSERT_LOCK_OBJECT`／`DELETE_LOCK_OBJECT` **从未部署**，报告却把 `ddic-helper-lock-object` 判为 `available`（`delete_ddic_object` 的 ENQU 分支同样被掩盖）——调用方看到可用能力名，调用时才拿到 `OPERATION_NOT_SUPPORTED`。**修订**：工具注册表每行新增第 8 列"该工具会派发的助手操作码"（如 `read_lock_object` → `READ_LOCK_OBJECT`；`delete_ddic_object` → 7 个 `DELETE_*`，因为它按 `objectType` 选择操作码），能力判定改为把所需操作码与助手自述清单逐条核对：全部命中 → `available`（`evidence.source = version-and-operation-check`，并给出逐工具 `toolObservations`）；部分命中 → **新增 `partial`**，逐个工具列出缺失操作码，已命中的工具不受影响（写侧缺失不再拖累可用的读侧）；全部缺失 → `unsupported`；协议低于最低版本仍是 `unsupported`（`version-check`）。两条**保持仅按版本判定**的路径会在 `evidence.detail` 里如实写明"结论仅基于协议版本"，不假装核对过：助手未发布操作码清单、注册表尚未登记所需操作码。`npm run matrix:check` 新增闸门：路由到 `Z_ORVANTA_MCP_DDIC_API` 的工具必须登记所需操作码。**缺陷 S1 / Q10（白名单绕过，High）**：D5-2 表白名单此前只守 RFC 后备路径（`read_abap_table`），`execute_data_query` 的**原生 ADT 主路径**把调用方 SQL 直接交给 `runQuery`，同一张被拒的表换个工具就能读到（既有单测正是用未入白名单的 `ZDATA` 取数，因此长期未被发现）。**修订**：主路径在**任何 SAP 访问之前**执行 `assertQueryTablesAllowed`，语句里每个 `FROM`／`JOIN` 目标都必须在白名单内（默认拒绝），且**表名无法静态枚举时同样拒绝**（新增稳定码 `TABLE_ALLOWLIST_UNVERIFIABLE`：动态表名、无 `FROM` 目标、逗号连接的多表清单）；子查询与显式 `JOIN` 会被枚举，字符串字面量先掩码（`WHERE MTEXT = 'FROM USR02'` 不会被误判为读 USR02）。**S3（发布溯源）**：`v0.46.1`–`v0.46.5` 已按 `release/INDEX.md` 记录的提交补打标签，且逐个提交内 `package.json` 与 `src/version.ts` 的版本号与标签一致；打包脚本新增闸门——正式产物要求 `v<版本>` 标签存在且指向当前 HEAD，否则在**创建任何产物之前**拒绝（候选产物 `-CandidateSuffix` 不受限），`BUILD-INFO.json` 新增 `sourceTag` 字段。**同时闭环 15:02 事件**：客户端报"工具清单仍无 `resume_ddic_table_activation`"经复核为**客户端会话内陈旧的工具目录**（会话早于该工具出现的 0.46.3），服务端 4847（0.46.6）`tools/list` 实测 132 项且含该工具与完整 schema，处置为客户端重连，非注册缺口（记录见 `.logs/mcp-incident-20260921-150254-001-...-closure.md`）。工具面不变：132 项 / 只读 77 项，能力组 18 组 / 50 工具。本版本已完成 `tsc` 构建、**750 项自动化测试（全部通过）**及 4 项静态检查；**未修改任何 SAP 对象、未部署助手、未操作传输**，故 R-1 只修"判定"而不改变助手侧缺失操作码的事实，状态为 `Partially Verified`。

2026-09-21 版本 `0.46.6` 确认并**以可证伪的回归守卫固定**「非活动 DDIC 定义的恢复工具在 MCP 工具注册表中公开且参数 schema 完整」，同时更正 0.46.5 对同一现象的调查结论。**起因**：`.logs/mcp-incident-20260921-141349-001-resume-ddic-tool-not-exposed.md` 记录 14:13 客户端在 `mcp__abap_fs_standalone__*` 清单中找不到 `resume_ddic_table_activation`，另一条工作流（`ZTPMC_TPRPI` 续建）因此停在恢复门槛。**只读复核结论：该工具在 0.46.5 已正确注册并公开**——注册表 `src/tool-registry.ts:213`（`dev` profile、`sap-helper-fallback` → `Z_ORVANTA_MCP_DDIC_API`、最低协议 `1.10`）、运行注册 `src/mcp.ts:445-452`、契约 `src/contracts.ts:907-919`；对运行中的 4848（dev，PID 31844，启动 14:17:30）与 4847（打包 0.46.5，PID 44996，启动 11:13:27）实测 `tools/list` **均返回 132 项工具且含该工具**，公开 JSON schema 为 `objectName`/`expectedInactiveFingerprint`/`packageName`/`transportNumber`/`confirmation`/`connectionId`（必填）+ `operationId`（可选，调用方生成、复用被阻断），`additionalProperties: false`，指纹约束 `^[a-f0-9]{64}$`，确认字面量 `RESUME_INACTIVE_ACTIVATION`，注解为 `readOnlyHint:false` / `destructiveHint:true` / `idempotentHint:false`。**因此该事件不是注册缺失，而是客户端会话的工具清单陈旧**（会话建立于 0.46.3 之前的服务实例，其后服务已多次升版）；处置为**重连/重载该 MCP 客户端后复测**，而**不是**改用 `create_ddic_transparent_table` 重建对象（读取响应明确禁止再次 create，且会重发定义）或 `abap_activate`（面向源码 URI，不是已存 DDIC 定义的激活）。**助手依赖已满足**：本轮只读 `get_capability_report(w200)` 实测 DDIC 助手自述 `maxProtocol: "1.10"`、24 个操作码、`sourceHash 780aa87d…`（`SOURCE|HASH` 未变），即 `RESUME_TRANSPARENT_TABLE_ACTIVATION`（1.10）已在 SAP 侧存在。**本次改动**：新增 `test/ddic-resume-tool-contract.test.ts`（3 例）——①注册表身份（助手、最低协议、路由、注解，且与建表工具的最低协议、与源码激活工具的路由**不同**，使"被替代"必然失败）；②契约入参齐备与必填性（缺任一必填项、错误确认字面量、非法指纹均被本地拒绝）；③HTTP `tools/list` 公开的完整 JSON schema（键集合、`required`、`additionalProperties`、确认字面量、指纹 pattern、注解）。**验证**：`npm test` **741/741 通过**；`format:check` / `lint` / `typecheck` / `matrix:check`（`132 tools, 77 read-only, 12 groups`；`18 capabilities over 50 tools`）全部通过；守卫测试经**变异校验**（改写确认字面量后 2 例失败、还原后 3 例通过）证明断言非空洞。工具面不变：132 项 / 只读 77 项。**未改动的边界（如实标注）**：本版本**未修改任何 SAP 对象、未部署助手、未操作传输**；助手侧的**锁对象写入**（`UPSERT_LOCK_OBJECT`/`DELETE_LOCK_OBJECT`）**仍未部署**，而能力报告当前把 `ddic-helper-lock-object` 判为 `available`（判定只看协议版本、不看助手自述的操作码清单）——该判定缺陷已立项修复（R-1），在此之前 `upsert_lock_object` 与 `delete_ddic_object(ENQU)` 不可依赖。本版本状态：**`Partially Verified`**（客户端重连后的真实调用、`ZTPMC_TPRPI` 续建属运行时验收，按工作区 §1.1 人工优先由测试人员执行）。

2026-09-21 版本 `0.46.5` 修复**陈旧助手被误报为能力不支持**（Medium）——这是 0.46.4 实测仍失败的直接原因。**根因（决定性证据）**：`get_capability_report(w200)` 实测已部署的 DDIC 助手自述 **`maxProtocol: "1.8"`**（22 个操作码，最高读操作为 `READ_SEARCH_HELP`）——即 **0.46.2 的 ENQU（1.9）与 0.46.4 的非活动读取（1.10）助手侧改动从未部署到 SAP**，全部只存在于 `scripts/bootstrap-sap-helper.ps1`。服务侧源码正确，但运行中的助手仍按旧逻辑以 `INACTIVE_VERSION_EXISTS` 拒绝，而该文案读起来像「该能力不存在」，掩盖了真实原因是**部署缺口**，可能把调用方引向重新 create 等错误方向。**修订**：`readDdic` 在收到 `INACTIVE_VERSION_EXISTS` 且 `metadata.INACTIVE` 非 `X`（即助手未执行新逻辑）时，改抛 `INACTIVE_VERSION_UNREADABLE`，明确给出助手自述协议、「低于 1.10」、这是部署缺口而非对象缺失、**不得重新 create**，并给出可执行的下一步；新增 `ddicHelperProtocol()` 从能力报告读取助手协议且**永不抛异常**（读不到降级为 `unknown`，不掩盖原始失败）。`capabilities.ts` 本已能正确判定该能力为 `unsupported`，本次让读取路径真正使用该结论。**实测（只读）**：`read_ddic_transparent_table(ZTPMC_TPRPI)` → `INACTIVE_VERSION_UNREADABLE`（准确指出 `protocol 1.8 < 1.10`）；`read_ddic_transparent_table(T000)` 回归对照正常返回定义与指纹，**未受影响**。**本版本只改善诊断、不解除阻塞**：读取非活动定义的能力物理上不存在于已部署助手，`ZTPMC_TPRPI` 仍不可读；彻底解除阻塞需部署助手（含 ENQU 1.9 与 `RESUME_TRANSPARENT_TABLE_ACTIVATION` 1.10），而 `ZORVANTA_MCP_CORE` 会被 `SELF_FUNCTION_GROUP_FORBIDDEN` 拒绝脚本写入路径，须沿用「in-SAP 报告 + 人工 F8」。工具面 132 项、只读 77 项。本版本已完成 `tsc` 构建、738 项自动化测试（全部通过）及 4 项静态检查；助手未部署，状态为 `Partially Verified`。

2026-09-21 版本 `0.46.4` 修复**发布物泄露真实连接配置**（安全，High）并让**非活动 DDIC 版本可被读取与恢复**。**缺陷⑤（发布物泄露真实连接，High）**：`0.46.3` 的 `release/orvanta-mcp-0.46.3-win-x64.zip` 内 `connections.json` 携带了**真实内网 SAP 主机与账号**（2026-09-22 依安全收口 R-19 脱敏：主机 `<internal-host>`、端口 `<port>`、用户名 `<account>`；客户端 `200`、语言 `ZH` 保留原记录不改），而非出厂占位符。取证链：`0.46.1` 的 release 目录配置写于 zip 生成之后（zip 干净）；`0.46.2` 配置写于 zip 之后 3 秒（zip 干净）；`0.46.3` 配置在 zip 生成前就已被改写，故泄露随包发出。打包器原先只做「脏工作树」检查，而 `connections.json` 在 `.gitignore` 中，被忽略的污染文件不会计入脏状态，因此该泄露可静默通过。修复：`package-windows.ps1` 增加**独立于来源的最后闸门**——压包前按结构化字段校验出厂配置必须是占位符（唯一 `w200` 连接、用户名 `DEVELOPER`、客户端 `200`/语言 `EN`、主机为保留域 `*.invalid`、不得含 `password` 属性），任一条不满足即**拒绝打包**。**缺陷⑥（非活动版本不可读，High）**：`read_ddic_transparent_table` 遇到只有非活动版本的对象时直接以 `INACTIVE_VERSION_EXISTS` 拒绝，调用方拿不到定义、版本或指纹，因而无法安全决定续建或恢复，只能手工回读 `DD02L/DD03L/TADIR`。修复：读取路径在 `lv_describe` 下以 `DDIF_TABL_GET state='N'` **显式读取非活动版本**（原先读 `state='A'` 只会返回活动版本，字段清单根本不对），并以 `ev_status='S'` + `INACTIVE_VERSION_DESCRIBED` + `INACTIVE=X` 返回该定义；服务侧据此返回 `status:"inactive"`、`active:false`、`definitionFingerprint` 与 `resumeTool` 提示。**缺陷⑦（续建缺少真实前置校验，Medium）**：`resume_ddic_table_activation` 原先只把调用方给的指纹原样回显，未做任何比对。现在先读回已存定义、本地复算指纹并与 `expectedInactiveFingerprint` 比对，不一致即 `INACTIVE_STATE_CONFLICT` 中止且不触碰对象；同时区分「无非活动版本」（`NO_INACTIVE_VERSION`）与助手报错。**已核实非缺陷**：`search_sap_locks` 返回 `HELPER_NOT_APPROVED` 属**设计如此的审批前置**，非代码缺陷——该工具依赖 MAINT 助手并要求状态目录中存在 `maintenance-diagnostic-approvals.json`，且 `enabledSources` 须显式包含 `SM12`；响应已如实标注 `status:"unavailable"`、`entries:null`、`provesSapLockOwnership:false`，未把「未审批」伪装成「无锁」。工具面 132 项、只读 77 项。本版本已完成 `tsc` 构建、738 项自动化测试（全部通过）及 4 项静态检查；**助手尚未部署到 SAP**，非活动读取与续建均无 SAP 运行时证据，状态为 `Partially Verified`。

2026-09-21 版本 `0.46.3` 修复 DDIC 建表失败时的**错误消息丢失、失败判定错误与恢复路径缺失**（四项确证缺陷），并新增受指纹保护的激活恢复工具。**缺陷①（错误消息丢失，High）**：`DDIC_SAVE_FAILED` 分支在 `ROLLBACK WORK` **之后**才用 `MESSAGE ID sy-msgid ... INTO ev_message` 取消息，而 `ROLLBACK WORK` 会重置 `sy-msgid/sy-msgno/sy-msgv1..4`，导致真实 DDIF 失败原因被替换为 SY 类分隔行（实测返回 `结束段落 004 ****…`）。修正为在 `DDIF_TABL_PUT` 返回后**立即**捕获消息四要素，失败时优先回显真实消息、回退到 `sy-subrc` 数值；因捕获用 `MESSAGE ... INTO` 会覆盖 `sy-subrc`，后续判定改用显式保存的 `lv_put_subrc`。**缺陷②（失败判定错误，High）**：原条件 `lv_activation_subrc <> 0 OR lv_active_state IS INITIAL AND lv_rc <> 0` 中 `AND` 优先于 `OR`，与书写意图不符；且 `lv_rc` 只在 `DD_TABL_ACT` 内赋值，而 `DDIF_TABL_PUT` 失败时该调用根本未执行，判定依赖一个从未更新的陈旧变量。修正为只依据真实子返回码并显式清零 `lv_rc`。**缺陷③（缺部分状态，Medium）**：失败回执原先只有一个 `outcomeMayBeUnknown`，调用方无法区分"完全未写入/已登记 TADIR/已存非活动版本/已激活但回读失败"，只能手工四路回读。现在失败时由助手当场回读 DD02L 与 TADIR，并在载荷中返回 `PHASE`（`inactive_saved`/`save_rejected`）、`PUT_SUBRC`、`ACT_SUBRC`、`ACT_RC`、`INACTIVE_EXISTS`、`TADIR_EXISTS`、`MSG_CLASS`、`MSG_NUMBER` 与 `RECOVERY_OPERATION`。**缺陷④（无恢复路径，Medium）**：读取路径在发现非活动版本时按设计拒绝（`INACTIVE_VERSION_EXISTS`），且不存在"继续激活既有非活动定义"的操作，事故中被完全阻塞。新增助手操作码 `RESUME_TRANSPARENT_TABLE_ACTIVATION`（协议 **1.10**）与服务侧工具 `resume_ddic_table_activation`：**不重发定义**，仅对已存储的非活动版本执行激活，激活前守卫 `DD02L.AS4LOCAL = 'N'` 必须存在（否则 `NO_INACTIVE_VERSION`），激活后回读活动表并如实声明 `definitionResent: false`、`automaticRetry: false`、`automaticRollback: false`。该工具单列能力组 `ddic-helper-table-activation-resume`——装配期校验器已拒绝将其并入协议 1.7 的表复合组，证明该约束有效。工具面 132 项、只读 77 项，能力组 18 组 / 50 工具。本版本已完成 `tsc` 构建、738 项自动化测试（全部通过）及 4 项静态检查；**助手尚未部署到 SAP**，因此新工具与修复后的错误诊断均无 SAP 运行时证据，状态为 `Partially Verified`。既有事故对象 `ZTPMC_TPRPI` 的部分状态**不因本版本改变**——本次修复的是工具的诊断与恢复能力，不是既有数据。

2026-09-20 版本 `0.46.2` 在 `0.46.1` 基线上落地 D6-2（锁对象）的**服务侧**，并修复助手脚本中三处字段级错误。**D6-2 锁对象**：新增 `read_lock_object` / `upsert_lock_object`，`delete_ddic_object` 的 `objectType` 扩展 `ENQU`，DDIC 助手新增 `ENQU` 分支（`READ_LOCK_OBJECT` / `UPSERT_LOCK_OBJECT` / `DELETE_LOCK_OBJECT`，协议 `1.9`，`PROTOCOL|MAX` 由 `1.8` 升至 `1.9`）；载荷新增 `L1`（`DD26E` 锁定表）与 `L2`（`DD27P` 锁定字段）两种两字符行类型。**脚本修复（均以 SAP 实证为准）**：①`DATA lv_kind` 由 `LENGTH 1` 改为 `LENGTH 2` —— 服务发送两字符行类型（`S1`/`S2`/`S3`/`L1`/`L2`），而脚本生成的助手只读一个字符，会把 `S1` 截断为 `S` 并落入 `WHEN OTHERS`，使任何携带子行的请求以 `PAYLOAD_INVALID` 失败；此前该陷阱仅存在于契约文本的警告中，根因从未落进脚本。②SHLP 写入分支的 `<ls_dd31v>-shpposition` 更正为 `shposition`（`DD31V` 实证为 `SHPOSITION`，单 `P`）。③ENQU 调用的 `TABLES` 形参名由 `dd26v_tab` 更正为 `dd26e_tab`，局部表类型由 `DD26V` 改为 `DD26E`（`DDIF_ENQU_GET`/`DDIF_ENQU_PUT` 接口实证形参为 `DD26E_TAB`，类型 `DD26E`；`DD26E` 比 `DD26V` 多 `ENQMODE` 字段）。工具面 131 项、只读 77 项，`docs/tool-index.md` 与 `contracts/tool-index.json` 已按注册表重新生成。本版本已完成 `tsc` 构建、738 项自动化测试（全部通过）及 4 项静态检查；**DDIC 助手尚未部署到 SAP**，因此两个新工具的 SAP 运行时验收（含 `DD_ENQU_GENERATE` 产出的 `ENQUEUE_*`/`DEQUEUE_*` 函数模块回读、负向用例）仍由测试人员按人工优先规则完成，本版本状态为 `Partially Verified`。
...
当前源码静态注册132个工具（其中只读77个），具体已验范围见上方当前基线。`get_capability_report`对指定连接执行有界只读探测，分别报告本地实现、已验证的原生ADT、SAP助手后备、不支持端点和无法在无目标条件下确认的能力；工具已注册或Discovery已公布不会被直接当作可用证明。`execute_data_query`只接受单条只读 `SELECT`，强制 `internal`模式、`rowRange`和1000行上限，不表示W200原生自由查询已恢复；`read_abap_table`是独立的结构化单表入口，最多500行。ATC不自动修复，ABAP Unit不自动激活，Dump和Trace只读取已有诊断数据。`abap_download`可将对象或包递归导出到显式绝对路径，默认拒绝覆盖现有目标；`adt_discovery_export`将四个Markdown文件写入便携目录的 `exports`。`manage_transport_requests`保留原工具的四个查询动作，用于读取用户传输、明细、对象清单和差异，不会创建、修改、删除或释放传输。`cleanup_transport_entries`是独立写工具，只允许从一个可修改任务中移除调用方明确列出的1至20条CTS对象记录；要求完整清单指纹、位置、操作ID和确认值，写后回读证明目标消失且其他条目未变化，不删除仓库对象，也不删除或释放请求。
