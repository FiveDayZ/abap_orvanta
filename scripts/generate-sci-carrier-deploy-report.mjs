...
// Deterministic build stamp. Re-running a program in SAP executes the source stored in SAP, NOT this
// file, so the operator needs a way to confirm the program carries the current revision. Derived only
// from the payloads, so regenerating the same bodies always yields the same stamp.
const payloadDigest = createHash("sha256")
  .update(fullSources.map((source) => source.join("\n")).join("\n"), "utf8")
  .digest("hex")
  .slice(0, 16)
...
CONSTANTS: c_group  TYPE c LENGTH 30 VALUE '${functionGroup}',
           ...
           c_digest TYPE c LENGTH 16 VALUE '${payloadDigest}'.
...
* If this line does not print the expected digest, the program in SAP still holds an older revision:
* re-paste the generated source and activate before running.
  WRITE: / 'payload digest:', c_digest.
  ULINE.
