...
param(
    [int]$Port = 0,
    [string]$ResultPath = (Join-Path $env:TEMP "sap-structured-rfc-validation-0180.json")
)
...
$stdout = Join-Path $env:TEMP "abap-mcp-0180-rfc-stdout.log"
$stderr = Join-Path $env:TEMP "abap-mcp-0180-rfc-stderr.log"
$probeStderr = Join-Path $env:TEMP "abap-mcp-0180-rfc-probe-stderr.log"
...
try {
    ...
    for ($attempt = 0; $attempt -lt 40; $attempt++) {
        ...
        if ($service.HasExited) {
            throw "0.18.0 service exited before becoming healthy: $(Get-Content -Raw $stderr -ErrorAction SilentlyContinue)"
        }
        try {
    ...
    if (-not $health) {
        throw "0.18.0 service did not start: $(Get-Content -Raw $stderr -ErrorAction SilentlyContinue)"
    }

    $resolvedResultPath = [IO.Path]::GetFullPath($ResultPath)
    Remove-Item -LiteralPath $resolvedResultPath, $probeStderr -Force -ErrorAction SilentlyContinue
    $probe = Start-Process -FilePath $node -ArgumentList (Join-Path $PSScriptRoot "probe-structured-rfc-validation.mjs") `
        -WorkingDirectory $projectRoot -Environment $processEnvironment -WindowStyle Hidden `
    ...
    if ($probe.ExitCode -ne 0) {
        ...
        throw "Structured RFC validation failed with exit code $($probe.ExitCode): $failureDetail"
    }
}
finally {
