...
const findAll = (predicate) =>
  before.map((l, i) => (predicate(l) ? i + 1 : 0)).filter((n) => n > 0)
...
const armFor = (predicate, label) => {
  ...
  assert.ok(hit, `could not identify the ${label} SHLP arm; following lines were:\n` +
...
const versionArm = armFor(
  (n) => n.startsWith("CONCATENATE ls_current_dd30v-as4date"),
  "version"
)
...
for (const a of anchors) assert.ok(a.at >= 1 && a.at <= before.length, `anchor ${a.name} out of range`)
...
assert.match(String(after.at(-1)).trim(), /^ENDFUNCTION\./i, "result no longer ends with ENDFUNCTION.")
assert.equal(
...
report.push("")
report.push("* GENERATED by scripts/generate-ddic-enqu-resume-deploy-report.mjs -- do not edit by hand.")
report.push(`* ${MARKER}`)
...
for (const line of after) report.push(`  CLEAR ls_new. ls_new-line = ${literal(line)}. APPEND ls_new TO lt_new.`)
report.push("")
...
report.push("  ENDLOOP.")
report.push("  IF lv_found = 'X'.")
report.push("    WRITE: / 'NOTHING TO DO: the deployed helper is already at protocol 1.10 or later.'.")
report.push("    RETURN.")
report.push("  ENDIF.")
report.push("")
...
report.push("  WRITE: / 'Next: call sap_helper_status ping, then get_capability_report to confirm'.")
