...
param(
    [int]$Port = 0,
    [string]$ResultPath = (Join-Path $env:TEMP "sap-rfc-validation-0170.json")
)
...
try {
    ...
    for ($attempt = 0; $attempt -lt 40; $attempt++) {
        ...
        if ($service.HasExited) {
            throw "0.17.0 service exited before becoming healthy: $(Get-Content -Raw $stderr -ErrorAction SilentlyContinue)"
        }
        try {
    ...
    if (-not $health) {
        throw "0.17.0 service did not start: $(Get-Content -Raw $stderr -ErrorAction SilentlyContinue)"
    }

    $resolvedResultPath = [IO.Path]::GetFullPath($ResultPath)
    Remove-Item -LiteralPath $resolvedResultPath, $probeStderr -Force -ErrorAction SilentlyContinue
    $probe = Start-Process -FilePath $node -ArgumentList (Join-Path $PSScriptRoot "probe-rfc-validation.mjs") `
        -WorkingDirectory $projectRoot -Environment $processEnvironment -WindowStyle Hidden `
    ...
    if ($probe.ExitCode -ne 0) {
        ...
        throw "RFC validation failed with exit code $($probe.ExitCode): $failureDetail"
    }
}
finally {
