...
report.push("        'no', lv_save_msgno.")
report.push("      WRITE: / 'Attempt 1   : v1=[', lv_save_v1, ']'.")
report.push("      WRITE: / 'Attempt 1   : v2=[', lv_save_v2, ']'.")
report.push("      PERFORM register_working_area.")
report.push("      PERFORM save_interface.")
report.push("    ENDIF.")
report.push("    IF lv_save_rc <> 0.")
report.push(
  "      WRITE: / 'Attempt 2   : working area save failed rc', lv_save_rc, 'id', lv_save_msgid,"
)
report.push("        'no', lv_save_msgno.")
...
report.push("        EXPORTING")
report.push("          funcname = lv_func")
report.push("          suppress_lock = 'X'")
report.push("        EXCEPTIONS")
report.push("          error_message = 1")
report.push("          OTHERS = 2.")
report.push("      IF sy-subrc <> 0.")
report.push("        WRITE: / 'ERROR: FUNCTION_DELETE failed', sy-subrc, sy-msgid, sy-msgno.")
...
report.push("          function_already_exists = 3")
report.push("          invalid_function_pool = 4")
report.push("          invalid_name = 5")
report.push("          too_many_functions = 6")
report.push("          OTHERS = 7.")
report.push("      IF sy-subrc <> 0.")
...
report.push("      SELECT SINGLE include FROM tfdir INTO lv_suffix")
report.push("        WHERE funcname = lv_func.")
report.push("      IF sy-subrc <> 0 OR lv_suffix IS INITIAL.")
report.push("        WRITE: / 'ERROR: TFDIR has no entry for the recreated', lv_func.")
report.push("        RETURN.")
report.push("      ENDIF.")
report.push("      CLEAR lv_name.")
report.push("      CONCATENATE 'L' c_group 'U' lv_suffix INTO lv_name.")
report.push("      WRITE: / 'Interface   : module recreated with', lv_added,")
report.push("        'canonical parameter(s) in the active version.'.")
report.push("    ELSE.")
report.push("      CALL FUNCTION 'DEQUEUE_ESFUNCTION'")
report.push("        EXPORTING")
report.push("          funcname = lv_func.")
