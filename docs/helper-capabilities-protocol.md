...
| `Z_ORVANTA_MCP_DYNPRO_API`（GUI/屏幕体）                                | 同上文件，非 DDIC 分支（`$repositoryFunctionSource` 之外的 GUI/屏幕体）                                                             | 该助手的仓库语义体已具备本协议；剩余部分尚未补齐                                                                                                                                   |
| ~~`Z_ORVANTA_MAINT_READ`~~ **已实现（未部署）**                         | `scripts/maintenance-diagnostic-source.mjs`；提交 `c988648`                                                                         | 3 个操作码；按 R3 走 JSON `payload`；按 R5 豁免 `CAPABILITIES` 的前置校验                                                                                                          |
| ~~`Z_ORVANTA_OPS_READ`~~ **已实现（未部署）**                           | `scripts/operational-log-source.mjs`；提交 `c988648`                                                                                | 6 个操作码（按生成特性门控）；同样适用 R3／R5                                                                                                                                      |
