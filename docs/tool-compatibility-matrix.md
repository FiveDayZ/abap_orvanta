...
> 当前口径（2026-09-23，`0.47.2`）：工具注册表静态注册 **137 个工具（只读 79 个，能力组 12 个）**，由 `npm run matrix:check` 与能力规格做一致性校验（实测输出：`tool index consistent: 137 tools, 79 read-only, 12 groups`；`helper capabilities consistent: 21 capabilities over 55 tools`）。下方按时间倒序保留各轮增量，其中"91 个工具""0.41.0 候选"等陈述均为各自时点的历史记录，不代表当前工具数。
