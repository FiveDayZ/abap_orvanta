...
> 当前口径（2026-09-22，`0.46.14` 工作树）：工具注册表静态注册 **134 个工具（只读 78 个，能力组 12 个）**，由 `npm run matrix:check` 与能力规格做一致性校验。下方按时间倒序保留各轮增量，其中"91 个工具""0.41.0 候选"等陈述均为各自时点的历史记录，不代表当前工具数。
