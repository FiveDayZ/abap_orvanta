...
| Waiting on each route | this repository 1, SAP-side helper + F8 6, operator approval 3, write authorisation (OP2) 4, multi-system configuration (OP3) 2, the platform (exempt) 1, - 2 |
...
| 10 | `runtime-resources` | Which work processes and sessions are live, what is on the application server's filesystem, and what performance data exists? | 4/4 | - | partial | 0/4 verified (pending: read_work_processes, read_user_sessions, read_file_system_directory, read_workload_directory) | SAP-side helper + F8 + operator approval |
